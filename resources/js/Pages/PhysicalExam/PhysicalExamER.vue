    ...
    >
        ...
        <template #formBody>
            <div class="grid grid-cols-1">
                ...
                <div>
                    <InputTextArea v-model="form.doctors_order" label="Doctor's Order" required :errorMsg="form.errors.doctors_order" @input="onFocusClearError('doctors_order');" />
                    <InputTextArea v-model="form.nurses_notes" label="Nurse's Notes" required :errorMsg="form.errors.nurses_notes" @input="onFocusClearError('nurses_notes');" />
                    <RadioButton v-model="form.cared" label="C.A.R.E.D." :options="CARED" required :errorMsg="form.errors.cared" @input="onFocusClearError('cared');" />
                    <div class="flex gap-2">
                        <InputText v-model="form.time_posted" label="Time Posted Signature" type="time" required :errorMsg="form.errors.time_posted" @input="onFocusClearError('time_posted');" />
                    </div>
                    <div class="flex flex-col p-1">
                        <label class="text-sm font-medium text-gray-800">Upload Attachment Files</label>
                        <input type="file" accept="image/jpeg" multiple class="p-2" @change=uploadImage>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        //         doctors_order: this.form.doctors_order,
        //         cared: this.form.cared,
        //         time_posted: this.form.time_posted,
        //         nurses_notes: this.form.nurses_notes,
    ...
    mounted() {
        if (this.action === 'update') {
            ...
            this.form = useForm({
                ...
                doctors_order: this.data.doctors_order,
                cared: this.data.cared,
                time_posted: this.data.time_posted,
                nurses_notes: this.data.nurses_notes,
                attachments: [],
                created_at: this.data.created_at,
                form_type: 'er',
            });
            this.form.attachments = this.data.attachments.map(attachment => attachment.image);
            this.formTitle = 'Update Physical Exam Emergency Room';
        }else{
            this.formTitle = 'New Physical Exam Emergency Room';
        }
